define(['angular', 'rd.services.DataSourceService','css!rd.styles.Area','css!rd.styles.Bootstrap', 'rd.services.EventService'], function () {
    ...
    areaModule.run(["$templateCache", function($templateCache) {
        ...
        $templateCache.put("city.html",
            '<div class="rdk-area-contain">\
                <ul class="nav nav-tabs">\
                    <li ng-if="!setting.disabled" ng-class="{active: vm.activeTab == 1}"><a ng-click="vm.activeTab = 1">{{vm.userArr[0].name || "省"}}</a></li>\
                ...
                <div class="tab-content tab-bordered">\
                    ...
                    <div class="tab-panel" ng-show="vm.activeTab == 2">\
                        <ul>\
                            <li ng-click="vm.selectAllProOrCity(\'全省\',\'lockCity\')" class="test-all"><a>全省</a></li>\
        ...
        function _link(scope,tElement,tAttrs,comboSelectCtrl) {
            ...
            function _init(){
                ...
                vm.dsProvinces = DataSourceService.create({
                    ...
                    url: serviceUrl,
                    resultHandler:_provincesResultHandler
                });
                //创建市数据源
                vm.dsCitys = DataSourceService.create({
                    id: 'dsCitys'+scope.$id,
                    url: serviceUrl,
                    resultHandler:_citysResultHandler
                });
                //创建区数据源
                vm.dsAreas = DataSourceService.create({
                    id: 'dsAreas'+scope.$id,
                    url: serviceUrl,
                    resultHandler:_areasResultHandler
                });
                //注册回调事件
                if(tAttrs.callback){
            ...
            function areaDataHandle(){  //处理选择结果，更新视图显示及返回数据结果
                ...
                if(scope.resultType=='string'){
                    ...
                    appScope[tAttrs.areaData]=obj;
                    obj=null;
                }
            }

            function _provincesResultHandler(){
                if(!scope.setting||!scope.setting.defaultData.province){
                    return
                }
