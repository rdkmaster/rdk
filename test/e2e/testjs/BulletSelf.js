...
describe('Bullet Self Test',function(){
    ...
    it('横向放置时候的滑块属性设置 direction',function(){
        ...
        expect(bar.getAttribute("width")).toBe("98%");
        expect(bar.getAttribute("height")).toBe("10px");
        ...
        expect(sliders.get(0).getAttribute("left")).toBe("47px");
        expect(sliders.get(1).getAttribute("left")).toBe("94px");
        expect(sliders.get(2).getAttribute("left")).toBe("235px");
        expect(sliders.get(3).getAttribute("left")).toBe("408.9px");
    });
    it('纵向放置滑块属性设置 direction',function(){
        ...
        expect(bar.getAttribute("height")).toBe("98%");
        expect(bar.getAttribute("width")).toBe("10px");
        //top值计算与横向的状态left 类似
        //[10,20,50,87] =>[49px,98px,245px,426.3px]
        var sliders=element.all(by.css(".vertical .progress .slider"));
        expect(sliders.get(0).getAttribute("top")).toBe("49px");
        expect(sliders.get(1).getAttribute("top")).toBe("98px");
        expect(sliders.get(2).getAttribute("top")).toBe("245px");
        expect(sliders.get(3).getAttribute("top")).toBe("426.3px");
    });
    ...
    it('进度minValue maxValue设置的效果',function(){
        ...
        expect(sliders.get(0).getAttribute("left")).toBe("58.75px");
        expect(sliders.get(1).getAttribute("left")).toBe("117.5px");
        expect(sliders.get(2).getAttribute("left")).toBe("176.25px");
        expect(sliders.get(3).getAttribute("left")).toBe("293.75px");
    });
