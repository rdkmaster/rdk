...
(function () {
    ...
    function getUserName(sessionId) {
        //从缓存中取
        var userName = Cache.aging.get("VmaxOperateLog_" + sessionId)
        if (!userName) {  //缓存中没有则发rest请求取并加入缓存
            ...
            if (userName) {
                Cache.aging.put("VmaxOperateLog_" + sessionId, userName);
                return userName
            } else {
                return ""
            }
    ...
    return function (userOpInfo, reqCtxHeader) {
        ...
        if(_.isString(userOpInfo)){
            Log.warn("userOpInfo参数json串形式已过时，请使用js对象");
            userOpInfo=JSON.parse(userOpInfo);
        }
