...
定义：
    ...
    function get(url, option); 或者 function get(url, param, option); 
...
参数传递时，应用可自行将参数拼装在url后面，调用 `Rest.get(url, option)`，也可以把参数独立开，然后调用 `Rest.get(url, param, option)`。

如果需要传递复杂结构，则建议调用 `Rest.get(url, param, option)`，RDK会对 `encodeURI()` 无法编码到的字符进行编码编码。这些字符有：`# & ' + =`。
