...
`i18n()` 函数可以一次国际化一个词条，也可以一次国际化一批词条，data.header是一个数组，可以直接传给 `i18n()` 函数，实现批量国际化。详情请参考[这里](/rdk_server/doc/service_api.html#i18n())。

### 测试国际化
RDK服务器默认会尝试从当前环境读取国际化信息，在Windows上由于读不到国际化信息，它会使用默认中文作为当前语言，使用[这里](/rdk_server/doc/manual.html#lang)介绍的方法可以改变当前语言。
...
你可以下载完成此步骤之后的[源码](08_i18n.zip)，解压到 `app/my_first_app` 下，[单击这里](/rdk_server/app/my_first_app/web/index.html)就可以打开它了。
